...
- `node_helper.js`, `MMM-LibraryMonitor.js`, `MMM-LibraryMonitor.css`
...
`opac`, `parser`, `config`, `ui`, `i18n`, `deps`, `devcontainer`, `node_helper`, `cli`
...
- Good: `fix(parser): treat a missing due-date cell as not overdue`
- Bad: `fix(parser): update open-account-parser.js`
- Bad: `fix(parser): various fixes and improvements`
...
```
feat(config)!: require an explicit accounts array
...
```
fix(opac): retry the account fetch once after a session cookie expires
fix(parser): stop misreading a German reservation date near midnight UTC
feat(config): support a shared libraryConfigFile across multiple accounts
feat(ui): show a book-cover placeholder while the OPAC image is loading
build(devcontainer): install the GitHub CLI
docs: correct the OPAC account setup instructions in the README
```
