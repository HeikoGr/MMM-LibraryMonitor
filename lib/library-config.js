...
function resolveLibraryConfig(moduleConfig = {}) {
  if (moduleConfig.libraryConfig && typeof moduleConfig.libraryConfig === "object") {
...
function resolveAccountConfigs(moduleConfig = {}) {
  const hasAccountList = Array.isArray(moduleConfig.accounts) && moduleConfig.accounts.length > 0;
  const accountEntries = hasAccountList ? moduleConfig.accounts : [moduleConfig];
  ...
  return accountEntries.map((entry, index) => {
    ...
    const adapter = validateSupportedLibrary(libraryConfig);

    return {
      id: accountConfig.id || `account-${index + 1}`,
      label: typeof accountConfig.label === "string" && accountConfig.label.trim() ? accountConfig.label.trim() : "",
...
function validateSupportedLibrary(libraryConfig) {
  ...
  // The adapter registry owns both the list of supported systems and the rules
  // for what a usable config looks like for each of them.
  const adapter = resolveAdapter(libraryConfig.api);
  adapter.validateConfig(libraryConfig);
