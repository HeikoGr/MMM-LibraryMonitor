 ...
 * unnoticed.
 */
async function reportTransportSecurity(
  accountUrl,
  tlsTrust,
  requestTimeout,
  logger,
  ...
  try {
    ...
    if (insecure && probe && !problem) {
      logger.warn("customssl disables certificate checks needlessly", {
        host: probe.host,
        problem:
          "the certificate is trusted; remove customssl from the library config",
      });
      return;
    }
    ...
    if (problem) {
      logger.warn("OPAC TLS certificate problem", {
        host: probe.host,
        problem,
        consequence: insecure
          ? "accepted only because customssl is set; prefer data.ca"
