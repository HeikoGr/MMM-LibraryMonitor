...
/**
 * Request options for one OPAC call. A caller's own headers (e.g. the login
 * POST's content-type) are added to the defaults, never replace them.
 * @param {object} options - The caller's fetch options
 * @param {object} dispatcher - undici dispatcher carrying the TLS trust
 * @param {number} requestTimeout - Timeout in milliseconds
 * @returns {object} fetch options
 */
function buildRequestInit(options, dispatcher, requestTimeout) {
  return {
    redirect: "follow",
    dispatcher,
...
function createHttpClient(tlsTrust, requestTimeout, cookieJar) {
  ...
  return async function request(url, options = {}) {
    return wrappedFetch(url, buildRequestInit(options, dispatcher, requestTimeout));
  };
}
