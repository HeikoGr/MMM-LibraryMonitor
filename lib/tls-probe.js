...
function inspectSocket(socket, host) {
  ...
  const hasCertificate =
    ...
    !!certificate.subject;
  const subject = hasCertificate
    ? certificateCommonName(certificate.subject)
    : null;
  const issuer = hasCertificate
    ? certificateCommonName(certificate.issuer)
    : null;
...
async function probeCertificate(urlString, options = {}) {
  ...
  const result = await new Promise((resolve) => {
    ...
    socket.on("timeout", () =>
      finish(failedProbe(host, "TLS handshake timed out")),
    );
