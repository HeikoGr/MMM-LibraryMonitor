...
function loadCa(value) {
  ...
  const resolvedPath = path.isAbsolute(value) ? value : path.resolve(__dirname, "..", value);
...
function getDispatcher(trust) {
  ...
  if (!dispatchers.has(key)) {
    dispatchers.set(
      ...
      new Agent({
        connect:
          trust.mode === TLS_MODE_PINNED ? { ca: trust.ca, rejectUnauthorized: true } : { rejectUnauthorized: false },
      }),
    );
  }
