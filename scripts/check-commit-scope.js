...
const RUNTIME_PATHS = [
  ...
  /^translations\//,
  /^node_helper\.js$/,
  /^MMM-LibraryMonitor\.js$/,
  /^MMM-LibraryMonitor\.css$/,
];
...
function main() {
  if (process.env.LIBRARYMONITOR_ALLOW_SCOPE_MISMATCH === "1") return;
...
${shown.map((file) => `    - ${file}`).join("\n")}${more > 0 ? `\n    ... and ${more} more` : ""}
  ...
  Deliberate exception:
    LIBRARYMONITOR_ALLOW_SCOPE_MISMATCH=1 git commit ...
`);
  process.exit(1);
}
